...
async function getProjetosDaDisciplina(idModulo) {
  // Alterado para ler projeto_url da tabela projetos
  const { data, error } = await supabase
    .from('projetos')
...
export async function generateMetadata({ params }) {
  ...
  const entity = await getEntidade(entidadeSlug);
  if (!entity) return { title: 'media3d' };

  const idEntidade = entity.id_entidade ?? entity.id;
  const prog = await getPrograma(idEntidade, programaSlug);
  if (!prog) return { title: `media3d — ${entidadeSlug}` };

  const modulo = await getModulo(prog.id_programa ?? prog.id, moduloSlug);
...
export default async function ModuloPage({ params }) {
  ...
  const entity = await getEntidade(entitySlug);
  if (!entity) notFound();
  const idEntidade = entity.id_entidade ?? entity.id;

  const prog = await getPrograma(idEntidade, progSlug);
  if (!prog) notFound();
  ...
  const codigoCurso = prog.codigo ?? progSlug.toUpperCase();
  const codigoMod   = modulo.codigo   ?? moduloSlug.toUpperCase();
  const nomeEscola  = entity.nome   ?? '';
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-[#0c0c0f]/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <a href="/admin/login" className="rounded-lg border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-medium text-white/50 transition hover:border-white/20 hover:text-white/80">
              Área Admin
            </a>
          </div>
        </div>
      ...
      <section className="px-6 pb-16 pt-20 text-center">
        <div className="mx-auto max-w-3xl">
          {(modulo.ano != null || modulo.semestre != null) && (
            <span className="mb-6 inline-block rounded-full border border-white/10 bg-white/5 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-[#4f9eff]">
        ...
        {projetos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {projetos.map((projeto) => (
              <div key={projeto.id_projetos} className="group relative flex flex-col overflow-hidden rounded-2xl border border-white/8 bg-[#13131a] transition-all hover:border-[#4f9eff]/40 hover:bg-[#181820]">
                
                {/* Imagem de Capa do Projeto (projeto_url) */}
                ...
                <div className="flex flex-1 flex-col justify-between p-6">
                  ...
                  <div className="mt-6 flex flex-col gap-3 border-t border-white/5 pt-4">
                    ...
                    <Link href={`/${entitySlug}/${progSlug}/${moduloSlug}/${projeto.id_projetos}`} className="mt-1 text-xs font-semibold text-[#4f9eff]/80 hover:text-[#4f9eff] transition-colors self-start">
                      Ver Projeto →
                    </Link>
                  </div>
                </div>
